...
class BST(object):
    ...
    def insert(self, data):
        ...
        """
        current = self.root
        def _find_parent(current, data):
            if current is None:
                return current
            if data ==current.data:
                raise ValueError(str(data) + " is already in the tree")
            if current.data < data and current.right is None:
                return current
            if current.data > data and current.left is None:
                return current
            if current.data > data: 
                return _find_parent(current.left, data)
            else:
                return _find_parent(current.right, data)

        new_node = BSTNode(data)
        
        parent = _find_parent(current, data)
    ...
    def remove(self, data):
        ...
        if to_delete == self.root:
            if to_delete.left is None and to_delete.right is None:
                ...
                to_delete.right.prev = None
                self.root = to_delete.right
            elif to_delete.right is None:
                to_delete.left.prev = None
                self.root = to_delete.left
            else:
                replacement = self.root.right
                while replacement.left is not None:
                    replacement = replacement.left
                new_root = BSTNode(replacement.data)
                if replacement == self.root.right:
                    replacement.left = self.root.left
                    replacement.prev = None
                    self.root = replacement
                    replacement.left.prev = replacement
                elif replacement.right is not None:
                    ...
                    replacement.prev.left = None
                    self.root.left.prev = new_root
                    self.root.right.prev = new_root
                    new_root.left = self.root.left
                    new_root.right = self.root.right
                    self.root = new_root
        else:
            if to_delete.left is None and to_delete.right is None:
                if to_delete.prev.left == to_delete:
                    to_delete.prev.left = None
                elif to_delete.prev.right == to_delete:
                    to_delete.prev.right = None
            elif to_delete.left is None:
                new_node = BSTNode(to_delete.right.data)
                new_node.prev = to_delete.prev
                new_node.left = to_delete.right.left
                new_node.right = to_delete.right.right
                if to_delete.right.left is not None:
                    to_delete.right.left.prev = new_node
                if to_delete.right.right is not None:
                    to_delete.right.right.prev = new_node
                if to_delete.prev.data > to_delete.data:
                    to_delete.prev.left = new_node
                else:
                    to_delete.prev.right = new_node
            elif to_delete.right is None:
                new_node = BSTNode(to_delete.left.data)
                new_node.prev = to_delete.prev
                new_node.left = to_delete.left.left
                new_node.right = to_delete.left.right
                if to_delete.left.right is not None:
                    to_delete.left.right.prev = new_node
                if to_delete.left.left is not None:
                    to_delete.left.left.prev = new_node
                if to_delete.prev.data > to_delete.data:
                    to_delete.prev.left = new_node
                else:
                    to_delete.prev.right = new_node
            else:
                current_node = to_delete.right
                while current_node.left is not None:
                    current_node = current_node.left
                self.remove(current_node.data)
                to_delete.data = current_node.data
...
def test_remove():
    ...
    b.remove(14)
    print b.root
    print b.root.right.prev
    print b.root.left.prev
    print b
